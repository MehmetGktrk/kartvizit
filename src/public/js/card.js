...
async function getCardData(){
    try{
        ...
        const result = await get(`/api/card/getVisitorCard/${id}`);



        cardData = result.cardData;

        document.title = `${cardData.isim} ${cardData.soyad} - Kartvizit`
...
async function generateVCard(cardData) {
  const person = {
    firstName: cardData.isim || '',
    lastName: cardData.soyad || '',
    prefix: cardData.onEk || '',

    organization: cardData.sirketAdi || '',
    title: cardData.unvan || '',
    role: cardData.pozisyon || '',

    workPhone: cardData.isTelefonu || '',
    cellPhone: cardData.kisiselTelefon || '',
    email: cardData.acikEmail || '',
    url: cardData.website || '',

    categories: cardData.kategori || '',

    address: {
      street: cardData.sokak || '',
      city: cardData.sehir || '',
      region: cardData.il || '',
      postalCode: cardData.postaKodu || '',
      country: "Ülke"
    },

    birthday: cardData.dogumGunu || '',
    note: cardData.not || '',
    photoUrl: cardData.profilResmi || '',
  };

  const {
    firstName, lastName, prefix,
    organization, title, role,
    workPhone, cellPhone, email, url,
  ...
  const vCardData = 
    ...
    FN:${[prefix, firstName, lastName].filter(Boolean).join(' ')}
    ORG:${organization}
    TITLE:${title}
    ROLE:${role}
    ...
    LABEL;TYPE=work:${address.street}, ${address.city}, ${address.region} ${address.postalCode}, ${address.country}
    BDAY:${birthday}
